...
public class Evaluator implements Visitor<Value> {
	...
	private int PairLen(PairVal pair) {
		int length = 0;
		PairVal current = pair;

		if (!(current.fst() instanceof Value.Null))
		{
			length++;
		}
		while (!(current.snd() instanceof Value.Null)) {
			length++;
			current = (PairVal) current.snd();
		}
		return length;
	}

	private boolean PairValEqual(PairVal first, PairVal second) {
		if (first.fst() instanceof Null && second.fst() instanceof Null) {
			return true;
		}
		NumVal firstFst = (NumVal) first.fst();
		NumVal secondFst = (NumVal) second.fst();
		if (firstFst.tostring().equals(secondFst.tostring())) {
	...
	public Value visit(LessExp e, Env env) { // New for funclang.
		...
		if (first instanceof NumVal && second instanceof NumVal) {
			NumVal first_num = (NumVal) first;
			NumVal second_num = (NumVal) second;
			return new Value.BoolVal(first_num.v() < second_num.v());
		}
		else if (first instanceof StringVal && second instanceof StringVal) {
			StringVal first_str = (StringVal) first;
			StringVal second_str = (StringVal) second;
			return new Value.BoolVal(first_str.v().compareTo(second_str.v()) < 0);
		}
		else if (first instanceof PairVal || second instanceof PairVal) {
			if (first instanceof Null) {
				return new Value.BoolVal(true);
			} else if (second instanceof Null) {
				return new Value.BoolVal(false);
			} else {
				PairVal first_pair = (PairVal) first;
				PairVal second_pair = (PairVal) second;
				return new Value.BoolVal(PairLen(first_pair) < PairLen(second_pair));
			}
		}
		else if (first instanceof Null && second instanceof Null) {
			return new Value.BoolVal(false);
		}
		return new Value.DynamicError("Something is broken " +  ts.visit(e, env));
	}
	...
	public Value visit(EqualExp e, Env env) { // New for funclang.
		...
		else if (first instanceof BoolVal && second instanceof BoolVal) {
			BoolVal first_bool = (BoolVal) first;
			BoolVal second_bool = (BoolVal) second;
			return new Value.BoolVal(first_bool.v() == (second_bool.v()));
		}
		else if (first instanceof PairVal && second instanceof PairVal) {
			PairVal first_pair = (PairVal) first;
			PairVal second_pair = (PairVal) second;
			if (PairLen(first_pair) == PairLen(second_pair)){
				return new BoolVal(PairValEqual(first_pair, second_pair));
			}
			return new BoolVal(false);
		}
		else if (first instanceof NumVal && second instanceof PairVal) {
			PairVal second_pair = (PairVal) second;
			if (PairLen(second_pair) == 1){
				return new BoolVal(second_pair.fst().tostring().equals(first.tostring()));
			}
			return new BoolVal(false);
		}
		else if (first instanceof PairVal && second instanceof NumVal) {
			PairVal first_pair = (PairVal) first;
			if (PairLen(first_pair) == 1){
				return new BoolVal(first_pair.fst().tostring().equals(second.tostring()));
			}
			return new BoolVal(false);
		}
		else if (first instanceof Null && second instanceof Null) {
	...
	public Value visit(GreaterExp e, Env env) { // New for funclang.
		...
		if (first instanceof NumVal && second instanceof NumVal) {
			...
			if (first instanceof Null && second instanceof PairVal) {
				return new Value.BoolVal(false);
			} else if (first instanceof PairVal && second instanceof Null) {
				return new Value.BoolVal(true);
			} else {
				PairVal first_pair = (PairVal) first;
				PairVal second_pair = (PairVal) second;
